...
from clique.Cluster import Cluster
from sklearn import metrics
from ast import literal_eval

#from Visualization import plot_clusters
from clique.Visualization import plot_clusters
...
# close to 100% of all time of run_clique is spent in this function
def get_dense_units_for_dim(data, prev_dim_dense_units, dim, tau):
    candidates = self_join(prev_dim_dense_units, dim) # fast
    prune(candidates, prev_dim_dense_units)           # fast

    # Count number of elements in candidates
    projection = np.zeros(len(candidates))
    for dataItem in data:
        for i, candidate in enumerate(candidates):
            data_in_projection = True
            for dim in candidate:
                if dataItem[dim[0]] != dim[1]:
                    data_in_projection = False
                    break
            if data_in_projection: projection[i] += 1
...
def run_clique(data, xsi, tau):
    ...
    # Pre-project all data
    projection = lambda x: x * xsi % xsi
    projecter  = lambda x: projection(x)
    processed_data = projecter(data)
    ...
    while (current_dim <= number_of_features) & (len(dense_units) > 0):
        ...
        dense_units = get_dense_units_for_dim(
            processed_data, dense_units, current_dim, tau)
